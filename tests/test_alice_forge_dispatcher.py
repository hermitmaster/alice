"""Tests for ``alice_forge.dispatcher`` — the v0 State Machine dispatcher.
...
def test_verify_skip_when_no_viewer_files_still_transitions(
    ...
    def pr_files(_repo: str, _n: int) -> list[str]:
        return ["src/alice_forge/dispatcher.py", "tests/test_sm_dispatcher.py"]
...
def test_thinking_spawn_started_prefix_distinct_from_worker_prefix() -> None:
    ...
    (centralized in alice_forge.comments per sub-issue 5) can disambiguate."""
...
def test_thinking_shim_module_is_importable_and_exits_cleanly() -> None:
    ...
    shim = importlib.import_module("alice_forge.thinking_shim")
...
def test_thinking_shim_main_exits_zero_with_valid_args(tmp_path) -> None:
    ...
    real entrypoint)."""
    from alice_forge import thinking_shim
...
def test_speaking_shim_module_is_importable_and_exits_cleanly() -> None:
    ...
    shim = importlib.import_module("alice_forge.speaking_shim")
    assert hasattr(shim, "main")
...
def test_speaking_shim_main_exits_zero_with_valid_args(tmp_path) -> None:
    ...
    from alice_forge import speaking_shim
...
def test_reviewing_pr_conflicting_tier2_spawns_on_rebase_failure(
    ...
    def attempt_rebase(_branch: str) -> dict:
        return {
            "ok": False,
            "reason": "auto-rebase failed at src/alice_forge/dispatcher.py",
        }
    ...
    assert spawn_calls == [
        (
            ...
            "jcronq/alice",
            "feat/work-173",
            "auto-rebase failed at src/alice_forge/dispatcher.py",
        )
    ]
    ...
    assert 'reason="auto-rebase failed at src/alice_forge/dispatcher.py"' in bodies[0]
...
def test_attempt_auto_rebase_aborts_on_conflict(
    ...
    rebase_stderr = (
        ...
        "CONFLICT (content): Merge conflict in src/alice_forge/dispatcher.py\n"
    )
    ...
    assert "src/alice_forge/dispatcher.py" in result["reason"]
...
def test_open_done_research_note_closes_on_worker_transition_audit_comment(
    ...
    :data:`alice_forge.dispatcher.SPAWN_ARTIFACT_HANDLERS`) to post a
